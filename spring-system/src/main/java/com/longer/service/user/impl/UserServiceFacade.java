...
import com.longer.service.user.UserService;
...
public class UserServiceFacade extends AbsUserService implements UserService {
	...
	public SysQueryService<?> queryUser() throws Exception {
		...
		if (POLICY_USER.equals(policyName)) {
			...
			// 资源调度器
			ResourceServiceFacade resourceServiceFacade = context.getBean(ResourceServiceFacade.class);
			// 用户和角色关联表调度器
			UserRoleFacade userRoleFacade = context.getBean(UserRoleFacade.class);
			queryPageService = context.getBean(QueryResourceList.class)
					.addUserId(userId).addAvailable(available).addIsLeaf(isLeaf).addFields(fields)
					.addResourceServiceFacade(resourceServiceFacade)
					.addUserRoleFacade(userRoleFacade);
			queryPageService.execute();
		}
