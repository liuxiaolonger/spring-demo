...
import com.etoc.service.sysparm.SystemService;
...
public class SystemServiceFacade extends AbsSystemService implements SystemService {
	 ...
	 * 新增和修改的对象
	 */
	private com.etoc.service.sysparm.vo.SystemInfo system;
	...
	@Override
	public SystemService setObject(com.etoc.service.sysparm.vo.SystemInfo system) {
		this.system = system;
		return this;
	}

	@Override
	public SysQueryService<SystemInfoResource> querySystem(String[] fields) throws Exception {
		SysQueryService<SystemInfoResource> sysQueryService = context.getBean(QuerySystem.class).setQueryParams(fields,
				systemId);
		// 执行器
	...
	public SysQueryService<?> querySystem(Integer pageNum, Integer pageSize, DataType dataType, String[] fields)
		...
		if (DataType.Page.equals(dataType)) {
			...
			sysQueryService = context.getBean(QueryPageSystem.class).setPageInfo(pageNum, pageSize, fields)
					.setQueryParams(systemName, systemKey);
		}
		// 全查询
		else if (DataType.List.equals(dataType)) {
			logger.info("系统参数全查询," + dataType);
			sysQueryService = context.getBean(QueryListSystem.class).setQueryParams(systemId, systemName, systemKey,
					systemValue, defaultFiedlds);
		} else {
			logger.error("参数错误");
	...
	public SysSaveService<?> saveSystem() throws Exception {
	...
	@Override
	@Transactional(rollbackFor = Exception.class)
	public BaseService moveSystem() throws Exception {
