...
public class RoleController
{
    ...
    {
        logger.info("开始查询url权限的信息 ,parames为pageNum:{},pageSize:{},roleName:{},dataType:{},fields:{},roleId:{},available:{}",
            pageNum,
            pageSize,
            roleName,
            dataType,
        ...
        try
        {
            RoleService roleService = context.getBean(RoleService.class).addQueryName(roleName).addQueryId(roleId).addAvailable(available);
    ...
    /* @ResponseStatus(HttpStatus.OK)*/
    public ResponseEntity<?> get(@PathVariable(name = "id", required = true) String id, String[] fields, Integer isLeaf)
    {
        ...
        try
        {
            RoleService roleService =
                context.getBean(RoleService.class).addQueryId(id).addIsLeaf(isLeaf).setFields(fields).addPolicyName(
                    AbsRoleService.POLICY_ROLE);
    ...
    public ResponseEntity<?> modify(@PathVariable(name = "id", required = true) String id, @RequestBody Role role)
    {
        ...
        {
            RoleService roleService =
                context.getBean(RoleServiceFacade.class).setObject(role).addQueryId(id).addPolicyName(
                    AbsRoleService.POLICY_ROLE);
